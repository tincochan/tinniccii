...
在endpoint的配置这里用到了```withSockJS()```，主要是面对一些或许[不支持websocket的浏览器时（几乎不存在）](https://developer.mozilla.org/zh-CN/docs/Web/API/WebSocket)，sockJS能够产生回调函数。
...
当然node.js环境下安装 sockjs-client 和 stompjs也是可以的。